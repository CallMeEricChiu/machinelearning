# -*- coding：utf-8 -*-
# Author:Eric Chiu Time:2019/8/7
import numpy as np
...
def anomalyDetection_example():
    ...
    plt.show()
    '''多元高斯分布函数，并可视化拟合的边界'''
    mu, sigma2 = estimateGaussian(X)  # 参数估计（求均值和方差） sigma2是2*1的列向量需转换为2*2矩阵
...
def visualizeFit(X, mu, sigma2):
    ...
    if np.sum(np.isinf(Z).astype(float)) == 0:  # 如果计算的为无穷，就不用画了
        ...
        CS = plt.contour(X1, X2, Z,10.**np.arange(-20, 0, 3))  # 画等高线，Z的值在10.**np.arange(-20, 0, 3)
        # plt.clabel(CS)
    plt.show()
